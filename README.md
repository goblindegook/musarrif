...
Running the ElixirFM comparison across the full 1013-verb dataset (`npm run debug:elixirfm`) currently scores **~98.4% agreement**, excluding the conjugations ElixirFM has no equivalent for.

92 verbs cannot be compared at all, and another 104 disagree with ElixirFM somewhere in their paradigm. Every discrepancy falls into one of the categories below, and each category has been adjudicated against Wiktionary. Four verbs disagree in two ways and are listed under both categories, so the rows do not sum to 104.
