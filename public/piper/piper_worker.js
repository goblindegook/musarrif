...
// piper_worker.js
async function phonemize(data, _onnxruntimeBase, modelConfig) {
...
async function init(data, phonemizeOnly = false) {
  ...
  if (Object.keys(cachedSession).length && !cachedSession[modelUrl]) cachedSession = {}
  ...
  if (Object.keys(modelConfig.speaker_id_map).length) feeds.sid = new ort.Tensor('int64', [speakerId])
  const {
    output: { data: pcm },
