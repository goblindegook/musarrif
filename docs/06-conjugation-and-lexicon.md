...
The home verb list's "Most common" sort reads `src/data/verb-frequency.json`, an ordered list of verb ids generated by a two-step offline pipeline from the CAMeL Lab MSA frequency list (release v1.0, not committed). Step 1 analyzes words seen at least 1000 times in the corpus (97.7% of tokens) with CAMeL Tools MLE, one type at a time, writing verb lemma counts to `.caches/frequency/verb-lemmas.tsv`. Step 2 maps those lemmas to verb ids (vocalised match, or unvocalised fallback, split evenly when ambiguous) and writes `src/data/verb-frequency.json`, most frequent first.
...
First run: install `uvx --from camel-tools camel_data -i disambig-mle-calima-msa-r13` (about 130 MB, one time). Verbs added after a build are unranked and listed at the end alphabetically until someone reruns both steps; a data test gates the output (at least 18 of 20 core verbs in top 100, كَتَبَ above كَتَّبَ).
