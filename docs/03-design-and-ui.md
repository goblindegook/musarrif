...
- **Borders carry structure**: 1px everywhere (2px only on exercise answer-feedback states and `LinkButton`); hover/focus shift border to accent instead of adding depth. Resting border contrast is deliberately soft (~2.8:1 on paper) — quiet until engaged. Hover/focus raises the border to cinnabar and the 3px accent focus outline carries the accessible affordance; the resting border does not target WCAG 1.4.11 on its own.
- **Typography**: `system-ui` for UI (intentional), Noto Sans Arabic (400–700) for Arabic. Uppercase + `0.08em` tracking on tabs and structural labels. Generous Arabic sizing — always the visual centrepiece. No decorative eyebrows, except the header's "Arabic Verb Conjugator" label: part of the logo lockup, hidden below 480px.
- **Verb pills**: active = cinnabar border + cinnabar text on every line (lemma, form, vowel pattern, gloss), no fill. Pills only; tabs and toggles keep their active fill.
- **Verb index rows** (browse list): ruled index, not cards. Hairline between rows, none under the last row. Order, left to right: LTR `gloss vowel form lemma ›`, Arabic UI `‹ vowel form lemma`. Lemma large and always flush right; gloss muted; muted chevron as the link cue. Hover: text + chevron cinnabar, no background, hairline unchanged.
