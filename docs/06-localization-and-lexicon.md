...
Four sources exist for `npm run add:tests -- <source> <slug>` (`wiktionary`, `elixirfm`, `qutrub`, `reverso`). Use them in this order, which is also the order `add:tests:all` falls back through:
