...
Four generators exist (`add:tests:wiktionary`, `add:tests:elixirfm`, `add:tests:qutrub`, `add:tests:reverso`). Use them in this order, which is also the order `add:tests:all` falls back through:
