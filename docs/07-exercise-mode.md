...
- `tenses` (0–4): gradually unlocks from active past to full passive set
- `pronouns` (0–3): from `3ms` to full inventory
- `diacritics` (0–2): `all` → `some` → `none`
- `forms` (0–9): form I only → all ten forms
...
**Component pattern**: `ExerciseMode` accepts a `generateExercise` prop (defaults to `nextExercise`) for test injection. Preserve this pattern when adding exercise types.
