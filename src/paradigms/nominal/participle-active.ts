...
export function deriveActiveParticiple(verb: Verb): string | null {
  const result = (() => {
    ...
    switch (verb.form) {
      ...
      case 4: {
        ...
        if (isInitialWeak && isFinalHamza) return [...prefix, c2, KASRA, seatedC3]
      ...
      case 5: {
        if (isFinalWeak) return [MEEM, DAMMA, TEH, FATHA, c1, FATHA, c2, TANWEEN_KASRA]
  ...
  return normalizeAlifMadda(result).join('').normalize('NFC')
}
