...
import { exerciseDiacritics, normalizeExercisePronoun, randomPronoun, randomTense, randomVerb } from '../dimensions.ts'
import {
  mixedWordDistractor,
  randomizeOptions,
...
function buildOptions(answer: string, word: string): readonly string[] {
  const generators = [
  ...
  return randomizeOptions(answer, generators, 4, [normalizeHamza])
}
