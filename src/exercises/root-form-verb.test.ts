...
describe('rootFormVerbExercise', () => {
  test('returns kind "rootFormVerb"', () => {
    expect(rootFormVerbExercise(INITIAL_DIMENSION_PROFILE).kind).toBe('rootFormVerb')
  })

  test('returns a non-empty Arabic word', () => {
    expect(rootFormVerbExercise(INITIAL_DIMENSION_PROFILE).word.length).toBeGreaterThan(0)
  })

  test('returns the correct translation key', () => {
    expect(rootFormVerbExercise(INITIAL_DIMENSION_PROFILE).promptTranslationKey).toBe('exercise.prompt.rootFormVerb')
  })

  test('returns promptParams with a form Roman numeral', () => {
    expect(rootFormVerbExercise(INITIAL_DIMENSION_PROFILE).promptParams?.form).toMatch(/^[IVX]+$/)
  })

  test('returns at least two options', () => {
    expect(rootFormVerbExercise(INITIAL_DIMENSION_PROFILE).options.length).toBeGreaterThanOrEqual(2)
  })

  test('all options are unique', () => {
    const { options } = rootFormVerbExercise(INITIAL_DIMENSION_PROFILE)
    expect(new Set(options).size).toBe(options.length)
  })

  test('correct answer is a valid index into options', () => {
    const { options, answer } = rootFormVerbExercise(INITIAL_DIMENSION_PROFILE)
  ...
  test('word matches the root of the selected verb', () => {
    ...
    const exercise = rootFormVerbExercise(easyProfile)
...
describe('rootFormVerbExercise with constraints', () => {
  test('attaches cardKey to returned exercise', () => {
    expect(rootFormVerbExercise(INITIAL_DIMENSION_PROFILE).cardKey).toMatch(/^rootFormVerb:[a-z]+:\d+$/)
  })
})
