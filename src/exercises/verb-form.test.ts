...
describe('generateFormExercise', () => {
  test('returns kind "form"', () => {
    expect(verbFormExercise(INITIAL_DIMENSION_PROFILE).kind).toBe('verbForm')
  })

  test('returns a non-empty Arabic word', () => {
    const { word } = verbFormExercise(INITIAL_DIMENSION_PROFILE)
    expect(word.length).toBeGreaterThan(0)
  })

  test('returns the correct translation key', () => {
    expect(verbFormExercise(INITIAL_DIMENSION_PROFILE).promptTranslationKey).toBe('exercise.prompt.verbForm')
  })

  test('returns exactly four options', () => {
    expect(verbFormExercise(INITIAL_DIMENSION_PROFILE).options).toHaveLength(4)
  })

  test('options are Roman numerals sorted in ascending form order', () => {
    const { options } = verbFormExercise(INITIAL_DIMENSION_PROFILE)
  ...
  test('correctAnswer is a valid index into options', () => {
    const { options, answer: correctAnswer } = verbFormExercise(INITIAL_DIMENSION_PROFILE)
  ...
  test('options[correctAnswer] matches the verb form', () => {
    const { options, answer: correctAnswer, word } = verbFormExercise(INITIAL_DIMENSION_PROFILE)
...
describe('formExercise difficulty', () => {
  ...
  test('pronouns:0 uses 3ms active past tense when random picks past (random=0)', () => {
    ...
    const { word } = verbFormExercise({ ...INITIAL_DIMENSION_PROFILE, pronouns: 0 })
  ...
  test('pronouns:2 diacritics:2 uses random pronoun and strips diacritics (random=0 → active past, 3ms)', () => {
    vi.spyOn(Math, 'random').mockReturnValue(0)
    const { word } = verbFormExercise({ ...INITIAL_DIMENSION_PROFILE, pronouns: 2, diacritics: 2 })
...
describe('verbFormExercise with constraints', () => {
  test('attaches cardKey to returned exercise', () => {
    expect(verbFormExercise(INITIAL_DIMENSION_PROFILE).cardKey).toMatch(/^verbForm:[a-z]+:\d+:[\w-]+:\w+$/)
  })
})
