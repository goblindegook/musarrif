...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      from: {
        pathNot: '^src/test/', // test-only helpers pull vitest internals
      },
      to: {
    ...
    },
    {
      ...
      to: {
        ...
        pathNot: '^virtual:', // vite virtual modules (e.g. virtual:pwa-register)
      },
    },
    {
    ...
    {
      name: 'logic-not-to-ui',
      severity: 'error',
      comment:
        'Paradigms and exercises are pure functions - no src/ui, no Preact, no goober. ' +
        'Return data and let the UI render it, or move the shared bit into src/primitives.',
      from: {
        path: '^src/(paradigms|exercises)/',
        pathNot: '[.](?:spec|test)[.](?:ts|tsx)$',
      },
      to: {
        path: ['^src/ui/', 'node_modules/(?:preact|goober)'],
      },
    },
    {
      name: 'paradigms-not-to-exercises',
      severity: 'error',
      comment:
        'Exercises build on paradigms, never the other way round. Grammar derivation must ' +
        'not know that exercise mode exists.',
      from: {
        path: '^src/paradigms/',
        pathNot: '[.](?:spec|test)[.](?:ts|tsx)$',
      },
    ...
    {
      name: 'primitives-are-a-leaf',
      severity: 'error',
      comment:
        'src/primitives holds generic helpers. Importing project modules into it makes them ' +
        'project-specific - put domain-aware helpers next to their domain instead.',
      from: {
        path: '^src/primitives/',
        pathNot: '[.](?:spec|test)[.](?:ts|tsx)$',
      },
    ...
    {
      name: 'atoms-not-to-higher-ui',
      severity: 'error',
      comment: 'Atomic design is one-way: atoms may not reach up into molecules, organisms or pages.',
      from: {
        path: '^src/ui/atoms/',
        pathNot: '[.](?:spec|test)[.](?:ts|tsx)$',
      },
      to: {
        path: '^src/ui/(?:molecules|organisms|pages)/',
      },
    },
    {
      name: 'molecules-not-to-higher-ui',
      severity: 'error',
      comment: 'Atomic design is one-way: molecules may not reach up into organisms or pages.',
      from: {
        path: '^src/ui/molecules/',
        pathNot: '[.](?:spec|test)[.](?:ts|tsx)$',
      },
      to: {
        path: '^src/ui/(?:organisms|pages)/',
      },
    },
    {
      name: 'organisms-not-to-pages',
      severity: 'error',
      comment:
        'Atomic design is one-way: organisms may not reach up into pages. Lift the shared ' +
        'piece into an organism or pass it in as a prop.',
      from: {
        path: '^src/ui/organisms/',
        pathNot: '[.](?:spec|test)[.](?:ts|tsx)$',
      },
      to: {
        path: '^src/ui/pages/',
    ...
    },
    {
      ...
      from: {
        ...
        pathNot: [
          ...
          '^src/test/', // test harness, never bundled
          '[.]d[.]ts$', // ambient type references
        ],
      },
      to: {
...
// generated: dependency-cruiser@18.2.0 on 2026-09-09T09:56:32.001Z
